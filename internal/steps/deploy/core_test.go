...
func TestDo(t *testing.T) {
	...
	if err := deploy.Do("", []deploy.Artifact{objs, obj}, step); err == nil || err.Error() != "source directory required" {
		t.Errorf("invalid error: %v", err)
	}
	if err := deploy.Do("testdata", []deploy.Artifact{objs, obj}, step); err == nil || err.Error() != "no resource set" {
		t.Errorf("invalid error: %v", err)
	}
	e, _ := env.NewValues("a", &asset.Resource{})
	step.Resource = e
	if err := deploy.Do("testdata", []deploy.Artifact{objs, obj}, step); err == nil || err.Error() != "missing deploy destination" {
		t.Errorf("invalid error: %v", err)
	}
	...
	os.Mkdir(dest, 0o755)
	if err := deploy.Do("testdata", []deploy.Artifact{objs, obj}, step); err != nil {
		t.Errorf("invalid error: %v", err)
	}
	obj.Files = append(obj.Files, "")
	if err := deploy.Do("testdata", []deploy.Artifact{objs, obj}, step); err == nil || err.Error() != "empty file not allowed" {
		t.Errorf("invalid error: %v", err)
	}
	obj.Files = []string{}
	obj.Files = append(obj.Files, "{{ $.Name }}")
	if err := deploy.Do("testdata", []deploy.Artifact{objs, obj}, step); err == nil || err.Error() != "unable to find source file: testdata/a" {
		t.Errorf("invalid error: %v", err)
	}
